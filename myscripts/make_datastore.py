...
def make_datastore(args, mode, data_range):
    ...
    for vid in data_range:
        res2d_path = os.path.join(res2d_dir, vid + '.npy')
        i3d_path = os.path.join(i3d_dir, vid + '.npy')
        relation_path = os.path.join(relation_dir, vid + '.npy')
        object_path = os.path.join(object_dir, vid + '.npy')
